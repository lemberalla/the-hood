...
Codex can recommend a loop shape before starting work by calling `thehood_recommend_loop`. The tool is read-only and returns a recommended recipe, recommended stack, draft completion contract, alternatives, editable card actions, `runAction`, and dashboard payload for a loop-plan card. Recommendation does not call providers, edit files, approve gates, or start schedules; accepting the card should invoke the returned runtime action.
