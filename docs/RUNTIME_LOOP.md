...
- invoking model-backed providers such as `chatgpt-web`, `chatgpt-atlas`, `claude-code`, or `codex-cli` for read-only repo work
- sending runtime-captured repo context to browser or API model providers such as `chatgpt-web` or `chatgpt-atlas`, unless the user's external transfer policy auto-approves the manifest
...
ChatGPT Web is wired through a user-configured bridge command. ChatGPT Atlas is represented as an enabled-by-default, setup-gated Computer Use bridge provider backed by the packaged `thehood-chatgpt-atlas-bridge` command. Real Atlas desktop use still requires a confirmed target window and a trusted local Computer Use controller command; deterministic tests can use the bridge's fake transport. API model providers are not wired yet. Local Codex CLI and Claude Code adapters are available through the same directive and response-validation contract.
